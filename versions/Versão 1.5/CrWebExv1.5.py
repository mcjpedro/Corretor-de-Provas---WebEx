...
class Ui_Programa(object):
    ...
    def diretorio(self):
        ...
        while  erro1 == 0:
            ...
            nmq = len(lines)
            if nmq > 5:
                messagebox.showwarning('Aviso', 'Este programa apenas corrige provas com menos de 5 questões')
                break
                                ...
                                while ex == 0:
                                    if len(lineopen[j+1]) <= int(splitvb-1) or lineopen[j+1][splitvb-1] != '|':
                                        if lineopen[j+1][0:len(lineopen[j+1])] != 'xxxxx':
                                            answer[cont2] = answer[cont2] + '(QUEBRA DE TEXTO)' + lineopen[j+1][0:len(lineopen[j+1])]
                                    else:
                                        ex = 1
                                    j += 1
                                cont2 += 1
